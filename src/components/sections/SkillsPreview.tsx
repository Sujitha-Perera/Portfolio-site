...
export function SkillsPreview() {
  ...
  const topSkills = [
    "NestJS", "React", "Next.js", "TypeScript", 
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          ...
          <FadeIn direction="right">
            <h2 className="text-3xl md:text-4xl font-bold tracking-tight mb-4 text-glow">
              <span className="text-4xl md:text-5xl font-extrabold text-white">Sujitha Perera</span><br/>
              Technical Arsenal
            </h2>
            <p className="text-muted-foreground mb-8 text-lg mt-2">
              I specialize in building robust full-stack architectures. From responsive frontends to scalable backend microservices, I leverage modern tools to deliver premium solutions.
            </p>
          ...
          <FadeIn direction="left" delay={0.2} className="relative">
            <div className="absolute inset-0 bg-gradient-to-tr from-cyan-500/20 to-emerald-500/20 rounded-3xl blur-3xl" />
                    ...
                    <div key={group.category} className="space-y-3">
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        {group.items.slice(0, 3).map((item) => (
                          <span key={item} className="text-xs text-muted-foreground bg-background/50 px-2 py-1 rounded border border-border/50">
                            {item}
                          </span>
                        ))}
