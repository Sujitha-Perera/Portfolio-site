...
export default function CertificationsPage() {
  return (
    <div className="min-h-screen py-24 relative overflow-hidden">
      ...
      />
      <div
        className="absolute top-1/3 -left-1/4 bg-emerald-500/10 rounded-full blur-[120px] pointer-events-none"
          ...
          {portfolioData.certifications.map((cert, index) => (
            <FadeIn key={cert.title} delay={0.1 * index}>
              <div className="glass-card flex flex-col sm:flex-row items-center sm:items-start gap-6 h-full p-8 rounded-3xl border border-border/50 group/card relative overflow-hidden">
                <div
                  className="absolute right-0 top-0 w-32 h-32 rounded-bl-full -z-10 group-hover/card:scale-125 transition-transform duration-700"
                  style={{
                    backgroundImage:
                      "linear-gradient(to bottom right, rgba(6, 182, 212, 0.05), rgba(16, 185, 129, 0.05))",
                  }}
                />
                ...
                <div className="flex-1 text-center sm:text-left">
                  ...
                  <Button
                    variant="outline"
                    size="sm"
                    className="glass rounded-full text-xs hover:bg-white/10"
                    disabled
                  >
                    Verify Credential{" "}
